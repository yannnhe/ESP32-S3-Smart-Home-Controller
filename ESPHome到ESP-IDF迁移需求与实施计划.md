...
| 21 | 厨房风扇 | 高电平开，上电安全关闭 |
| 17 | 厨房蜂鸣器 | 低有效，上电保持关闭 |
| 48 | 阳台窗舵机 | 50 Hz、14 位 PWM；关闭/打开 duty 为 1250/2048（约 7.6%/12.5%） |
...
- 已完成三个 MQTT Switch：卧室风扇 GPIO12、厨房风扇 GPIO21 为高有效，厨房蜂鸣器 GPIO17 为低有效；启动时均强制关闭，命令经执行任务写入 GPIO 后回传 retained `ON`/`OFF` 状态。
- 已完成阳台窗 MQTT Switch：GPIO48 输出 50 Hz、14 位 PWM，`ON` 全开（duty 2048，约 12.5%）、`OFF` 全关（duty 1250，约 7.6%）；执行任务约 1 秒平滑移动、到位后停 PWM。每次启动均实际执行关窗；动作成功后发布 retained `OFF`，不使用 NVS 保存或恢复窗户状态。
...
- 窗户不保存或恢复位置；每次启动均由执行任务执行关窗，且不在命令接收时提前报告成功。
...
- 上电先将灯带、风扇、蜂鸣器置于安全关闭状态；阳台窗随后由执行任务实际执行关窗，并在成功后向 HA 报告关闭。
...
| P3 | 验证风扇、蜂鸣器与命令路由 | 命令、硬件状态和 HA 状态闭环一致 |
| P4 | 验证阳台窗舵机与启动关窗 | 动作安全、到位后停 PWM、每次重启均实际关窗且 HA 最终显示关闭 |
