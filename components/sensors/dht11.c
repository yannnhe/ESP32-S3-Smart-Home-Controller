...
static void dht_sampling_task(void *argument)
{
    ...
    while (true) {
        for (size_t i = 0; i < DHT_SENSOR_COUNT; ++i) {
            int16_t humidity_tenths;
            int16_t temperature_tenths;
            esp_err_t err = dht_read_data(DHT_TYPE_DHT11, s_dht11_sensors[i].gpio,
                                          &humidity_tenths, &temperature_tenths);
            ...
            if (err != ESP_OK) {
                ESP_LOGW(TAG, "%s DHT11 read failed: %s", s_dht11_sensors[i].name,
                         esp_err_to_name(err));
                if (should_publish_unavailable) {
                    ha_discovery_publish_state_group(s_dht11_sensors[i].state_group);
                }
            } else {
                ha_discovery_publish_state_group(s_dht11_sensors[i].state_group);
            }
...
esp_err_t dht11_init(void)
{
    ...
    for (size_t i = 0; i < DHT_SENSOR_COUNT; ++i) {
        ...
        esp_err_t err = ha_discovery_register_state_group(&state_group_config,
                                                           &s_dht11_sensors[i].state_group);
    ...
    if (xTaskCreate(dht_sampling_task, "dht_sampling", 4096, NULL,
